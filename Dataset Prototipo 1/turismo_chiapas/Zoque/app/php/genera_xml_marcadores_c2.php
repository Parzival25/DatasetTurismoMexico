<?php
  $con = mysql_connect("localhost","hgcrespo_tz","zoque");
  if (!$con) die('!!! Conexión fallida !!!: ' . mysql_error());
  
  mysql_select_db("hgcrespo_turismo_zoquetsotsil", $con);
  ...
  //$sql = "select * from atomos where ATG_C2 like '2'";
  $sql = "select * from atomos";
 ...
 // exit;
  ...
  if ($b==0)  /* no hubo resultados */
  {
	...
	// no debe llevar acentos en el servidor
	// no debe llevar acentos en el servidor
	$newnodo -> setAttribute("nombre", "No existen sitios");  // no debe llevar acentos en el servidor
