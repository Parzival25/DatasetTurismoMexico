<?php
  $con = mysql_connect("localhost","hgcrespo_ts","sancris");
  if (!$con) die('!!! Conexión fallida !!!: ' . mysql_error());
  
  mysql_select_db("hgcrespo_turismo_sancristobal", $con);
  ...
  //{ 
    //$cadena = $row['AT_C2']; /* cadena de la categorías*/
	//echo "<pre>";
	//print_r($row);
	//echo "</pre>";
  //}	
  ...
  while ($row = @mysql_fetch_assoc($resultado))
  { 
    $cadena = $row['IN_C2']; /* cadena de la categorías*/
    ...
    while($cadena[$i]!=".")
	{
	 ...
	 /* echo "<pre>";
	  ...
	  if(strcmp($categoria, $_GET["q"]) == 0)
	  {
        ...
        $newnodo -> setAttribute("id", $row['IN_ID']);
	    $newnodo -> setAttribute("nombre", $row['IN_NOMBRE']);
        $newnodo -> setAttribute("lat", $row['IN_LATITUD']);
        $newnodo -> setAttribute("lng", $row['IN_LONGITUD']);       		
		$b=1;
      }
  ...
  if ($b==0)  /* no hubo resultados */
  {
	...
	// no debe llevar acentos en el servidor
	// no debe llevar acentos en el servidor
	// no debe llevar acentos en el servidor
	$newnodo -> setAttribute("nombre", "No existen sitios");
    $newnodo -> setAttribute("lat", "16.172473");
    $newnodo -> setAttribute("lng", "-92.620239");	
  }  
