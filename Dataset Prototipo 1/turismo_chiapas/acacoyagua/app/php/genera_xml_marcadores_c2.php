<?php
 ...
 // exit;
    ...
    while($cadena[$i]!=".")
	{
	  while($cadena[$i]!="," and $cadena[$i]!=";")  /* enviar la clasificación de manera individual a la subcadena para después compararla */
	  {
	    if ($cadena[$i]!=".")
		{
		  ...
		  $i++;
		/*
		echo "<pre>";//para imprimir un dato para comprobar is esta bien o no
		print_r($row);
		echo "</pre>";
		exit;
		*/		
  ...
  if ($b==0)  /* no hubo resultados */
  {
	...
	// no debe llevar acentos en el servidor
	// no debe llevar acentos en el servidor
	$newnodo -> setAttribute("nombre", "No existen sitios en esta categoría");  // no debe llevar acentos en el servidor
    $newnodo -> setAttribute("lat", "15.341223");
    $newnodo -> setAttribute("lng", "-92.676118");	
  }  
